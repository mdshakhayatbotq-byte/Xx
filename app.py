import os, sys, sqlite3, zipfile, subprocess, signal, shutil, psutil, time, datetime, threading
...
# Cache of psutil.Process objects keyed by pid, so cpu_percent() has a
# previous sample to compare against. Without this, a fresh psutil.Process()
# is created on every request and cpu_percent(interval=None) always reports
# ~0% since it has nothing to compare to.
_cpu_proc_cache = {}

def get_cpu_ram(pid):
    try:
        proc = _cpu_proc_cache.get(pid)
        if proc is None or proc.pid != pid:
            proc = psutil.Process(pid)
            proc.cpu_percent(interval=None)  # prime the first sample
            _cpu_proc_cache[pid] = proc
            return "0%", f"{proc.memory_info().rss / (1024 * 1024):.1f}MB"
        cpu = proc.cpu_percent(interval=None)
        ram = proc.memory_info().rss / (1024 * 1024)
        return f"{cpu:.1f}%", f"{ram:.1f}MB"
    except Exception:
        return "0%", "0MB"

def clear_cpu_cache(pid):
    if pid in _cpu_proc_cache:
        del _cpu_proc_cache[pid]

def run_pip_install(path, log_file_path):
    """Runs pip install -r requirements.txt in the background, in a thread so
    the request returns immediately. Some hosting images (newer Debian/Ubuntu
    with PEP 668) refuse global pip installs with an "externally-managed-
    environment" error — if that happens we automatically retry once with
    --break-system-packages so installs don't silently fail on those hosts."""
    def worker():
        base_cmd = [sys.executable, '-m', 'pip', 'install', '-r', 'requirements.txt']
        f_log = open(log_file_path, 'a')
        try:
            proc = subprocess.Popen(base_cmd, cwd=path, stdout=f_log, stderr=f_log, env={**os.environ, 'PYTHONUNBUFFERED': '1'})
            proc.wait()
            if proc.returncode != 0:
                f_log.write("\n⚠️ Install failed — retrying with --break-system-packages (host looks like it uses an externally-managed Python)...\n")
                f_log.flush()
                proc2 = subprocess.Popen(base_cmd + ['--break-system-packages'], cwd=path, stdout=f_log, stderr=f_log, env={**os.environ, 'PYTHONUNBUFFERED': '1'})
                proc2.wait()
                f_log.write("\n✅ Install finished.\n" if proc2.returncode == 0 else "\n❌ Install still failed — check the errors above.\n")
            else:
                f_log.write("\n✅ Install finished.\n")
            f_log.flush()
        finally:
...
def create_app():
    ...
    def delete_bulk(folder):
        ...
        cur_startup = srv['startup'] if srv else None
        ...
        for name in names:
            ...
            # If the deleted file was the configured startup file, reset to the
            # default so a future Start doesn't stay pointed at something gone.
            rel = (sub_path + '/' + name).strip('/')
            if cur_startup and cur_startup in (rel, name):
                db.execute('UPDATE servers SET startup=? WHERE folder=?', ('main.py', folder))
                db.commit()
        db.close()
    ...
    def rename_file(folder):
        ...
        if os.path.exists(old_p):
            ...
            # If this file was set as the startup file, keep the DB pointer in sync —
            # otherwise a rename silently breaks Start/Restart (it'll fall back to
            # auto-creating a blank placeholder under the old name instead).
            old_rel = (sub_path + '/' + old_name).strip('/')
            new_rel = (sub_path + '/' + new_name).strip('/')
            db = get_db()
            srv = db.execute('SELECT startup FROM servers WHERE folder=?', (folder,)).fetchone()
            if srv and srv['startup'] in (old_rel, old_name):
                db.execute('UPDATE servers SET startup=? WHERE folder=?', (new_rel, folder))
                db.commit()
            db.close()
            return jsonify({'status': 'success'})
    ...
    def server_action(folder, act):
        ...
        if act == 'install':
            ...
            if os.path.exists(req_path):
                ...
                run_pip_install(path, log_file_path)
                db.close()
        ...
        if act in ['start', 'restart']:
            ...
            f_log = open(log_file_path, 'a')
            # -u = unbuffered stdout/stderr. Without this, Python fully-buffers output when it's
            # redirected to a file (not a TTY) — so real prints/errors from the user's bot sit in
            # memory and never reach console.log until the process exits. This is what makes the
            # live console feel "fake" compared to running it directly in Termux.
            proc = subprocess.Popen(['python3', '-u', startup_file], cwd=path, stdin=subprocess.PIPE, stdout=f_log, stderr=f_log, preexec_fn=os.setsid, text=True, bufsize=1,
    ...
    def set_startup(folder):
        d = request.json or {}
        cmd = d.get('file') or d.get('command') or ''
        if not cmd:
            return jsonify({'status': 'error', 'msg': 'No file specified'}), 400
        db = get_db()
